    ...
    Aluno.find({'nome': {$regex: nomeAluno, $options: 'i'}, 'cpf': {$regex: cpfAluno, $options: 'i'}, 'mae.nome': {$regex: nomeMae, $options: 'i'}}).lean().then((alunos) => {
      ...
      if(alunos.length){
        for(let i=0; i<alunos.length; i++){
          stringResponse += "<tr><td><a href='/alunos/editar/"+alunos[i]._id+"'>"+alunos[i].nome+"</a></td><td>"+(alunos[i].nascimento.getDate()+1)+"/"+(alunos[i].nascimento.getMonth()+1)+"/"+alunos[i].nascimento.getFullYear()+"</td><td>"+alunos[i].mae.nome+"</td><td>"+alunos[i].cpf+"</td></tr>"
        }
      }else{
...
router.post('/cadastro/edit', (req,res)=> {
  ...
  Aluno.findByIdAndUpdate(req.body.idAluno, editAluno, {'runValidators': true}, (err) => {
    if(err){
      res.send(err)
    }else{
